...
public class LimitedVisibilityLayout implements SLayoutManager {
     ...
     * @param minimumLayoutSize the minimum size of the layout on the horizontal/vertical axis. This axis depends on the
     *                          vertical parameter
     * @param vertical          if true, the layout will place its elements vertically, using minimumLayoutSize as its
     *                          horizontal minimum size. If false, the layout will place its elements horizontally,
     *                          using minimumLayoutSize as its vertical minimum size.
     */
    public LimitedVisibilityLayout(int nrOfVisibleElts, int minimumLayoutSize, boolean vertical) {
        assert nrOfVisibleElts > 0;
        this.nrOfVisibleElts = nrOfVisibleElts;
        this.minLayoutLongSize = minimumLayoutSize;
    ...
    public void recalculateProperties() {
        ...
        if (currentInd > nrOfTotalElts - nrOfVisibleElts) {
            currentInd = nrOfTotalElts - nrOfVisibleElts;
        }
        ...
        int endInd = nrOfVisibleElts + currentInd;
        ...
        for (int i = endInd; i < nrOfTotalElts; i++) {
            elements.get(i).setVisible(false);
        }
    }

    /**
     * shift the view to the given start index of visible elements. Calls {@link #recalculateProperties()} if
     * necessary.
     ...
     * necessary.
     * @param value the new first index. Indices {@code value} to {@code value + nrOfVisibleElts} will be shown
     */
    public void setAsFirstVisible(int value) {
        if (value != currentInd) {
            currentInd = value;
            recalculateProperties();
        }
    ...
    public void placeComponents(Vector2ic position, Vector2ic dimensions) {
        ...
        int endInd = currentInd + nrOfVisibleElts;
        for (int i = currentInd; i < endInd; i++) {
            SComponent elt = elements.get(i);
